...
送信前に本文をはてなブログの Markdown 表示へ合わせて整形する．画像処理より前に適用する．はてなの Markdown では段落内の単純な改行が不要な半角スペースになるため，著者が読みやすく折り返したドラフトを表示に合う形へ変換する．

- 段落（連続する非空行）内の改行を取り除き，1 行へ結合する．
- 段落の末尾へ半角スペースを 2 つ付け，明示的な改行とする．
- ブロックの境界へ区切り行（全角スペース 1 つに続けて半角スペース 2 つ）を挟み，余白を確保する．対象は段落どうし，段落から見出しへの境界，および箇条書き・リスト・埋め込み・画像の前後とする．見出しから本文への境界など余白が不要な箇所では挟まない．
- 単独行のリンク（Markdown リンクまたは裸 URL）はカード型とみなし，埋め込み記法 `[URL:embed:cite]` へ変換する．文中のインラインリンクは対象外とする．
- 先頭の導入見出し（`導入`・`はじめに` の h2）は落とし，本文から始める．ドラフトでは構成のため残し，送信時に落とす．
- 見出し・箇条書き・画像・コードフェンス・引用・HTML・表の中身は変換しない．
