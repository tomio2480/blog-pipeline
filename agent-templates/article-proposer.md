...
`themes` は記事レベルタグであり，後段の `article-drafter` が本文化の steering 入力として読む．後段へ橋渡しするため outline のフロントマターへ必ず永続化する．詳細は「記事レベルタグ（ `themes` ）の引き継ぎ」を参照する．
...
- 入力素材に無い情報を補って章を設けない
- 1 つの素材を無理に 7 章以上に分割しない
- 人名を章タイトルや要旨に含めない．第三者の名前が必要な場合は **役割で言い換える**．例として「登壇者」「主催者」のような語へ変える

## 記事レベルタグ（ `themes` ）の引き継ぎ

`themes` は記事の力点を示す入力である．後段の `article-drafter` が本文化の steering として読む．汎用ひな形はこの引き継ぎ規約のみを定める．

- 採用済み記事案を入力とした場合，記事案フロントマターの `themes` をそのまま outline へ引き継ぐ．
- 補正済み素材を直接入力とした場合，素材に `themes` は無い．この経路での値の決め方は個人化版（利用者の Subagent ファイル）が定める．
- いずれの経路でも，得られた `themes` は outline のフロントマターへ必ず永続化する．

どのタグを採るかの判断は個人化版の責務とする．汎用側は規約と永続化のみを担い，個人色を含めない．
...
5. フロントマターを付けて `drafts/<YYYY-MM-DD>-<slug>-outline.md` へ書き出す
   - `source_materials`：採用済み記事案を入力とした場合はその `source_materials` を引き継ぐ．補正済み素材を直接指定した場合はそのパスを設定する
   - `themes`：「記事レベルタグ（ `themes` ）の引き継ぎ」に従い永続化する

`slug` は仮タイトルから不正文字（スペース・記号等）をハイフンに置換した文字列とする．最大 80 文字に切り詰める．
