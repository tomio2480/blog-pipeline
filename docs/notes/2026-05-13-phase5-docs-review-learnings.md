...
追加した文を見やすさのために 2 行で折り返したが，textlint は 98 字 1 文と判定して
...
bash の `--field body="..."` にバッククォートを含めると，bash はコマンド置換（`` `cmd` ``）と解釈して内容が欠落する．

**解決策**：Python で `{"body": "..."}` の JSON ファイルを生成し，
...
- **`gh pr comment` コマンド**：PR 全体への一般コメント用で，
  inline review comment への Reply には使えない．
