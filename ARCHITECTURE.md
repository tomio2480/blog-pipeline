...
| 記事案クラスタリング | Skill 内（メイン会話）| Opus | 文脈横断と判断はメイン会話の Opus 本体で行う |
...
`scripts/parse_enex.py` は標準ライブラリのみで実装する．サードパーティ依存を導入しない方針は，OSS 利用者が `pip install` 無しで動かせる状態を維持するためである．
...
`<YYYY-MM-DD>-<title>.md` 形式とし，以下のサニタイズを順に適用する．

- Unicode 正規化（NFC 固定．OneDrive 同期事故防止）
- Windows 不正文字（`\` `/` `:` `*` `?` `"` `<` `>` `|`）と制御文字をハイフンへ置換
- Windows 予約名（CON / PRN / AUX / NUL / COM1-9 / LPT1-9）はアンダースコアを末尾に付加
- 連続ハイフンと連続空白を 1 つへ圧縮
- 末尾のピリオド・空白・ハイフンを除去
- 最大長 80 文字（多バイト文字を含んでも OneDrive と Git の両方で安全な範囲）
- 同一ファイル名の衝突は連番サフィックス（`-2`，`-3`）で回避
...
- 個人情報・実名・固有名詞辞書・原稿・録音データを含めない（`materials/` 等は `.gitignore` で除外する）
- API キー・トークンを含めない
...
本リポジトリ（Public）は仕組みのみを提供する．生成物・素材・原稿などのデータは個人化版（Private）リポジトリへ集約する．配布方式は複数案を検討した．当面は D2 を採用する．D2 では，個人化版を本リポジトリの兄弟ディレクトリとして clone する．そのうえで CLI を個人化版の作業ディレクトリから呼び出す．
