...
利用者はリポジトリをフォークまたは clone のうえ，個人化版（文体プロファイル・固有名詞辞書・原稿）を別の Private リポジトリで持つ運用を想定する．
...
利用者の Private リポジトリは README やコミットメッセージで「個人化版」「Private リポジトリ」などの汎用呼称で参照する．
...
モデル指定は本ファイルおよび各 Subagent ひな形ファイル（`agent-templates/*.md`）で行い，世代交代に追随できる構造を保つ．具体的には Claude Code のエイリアス `opus`／`sonnet`／`haiku` を用い，バージョン番号までは固定しない方針．エイリアスは推奨バージョンへ自動的に解決され，世代更新に伴って上位版へ切り替わる．

参考：本ファイル更新時点（2026-04-29）のエイリアス解決先は `opus` → Opus 4.7，`sonnet` → Sonnet 4.6 が確認されている．`haiku` も同様に推奨バージョンへ解決される．再現性が必要な場面では環境変数 `ANTHROPIC_DEFAULT_OPUS_MODEL` 等でフルモデル名にピン止めできる．
