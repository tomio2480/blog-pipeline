...
本ファイルは Claude Code が `blog-pipeline` リポジトリで作業する際に常設で参照する指示である．本リポジトリは Public な OSS であり，再利用可能な汎用部分のみを集約する．個人情報・固有名詞辞書・原稿は本リポジトリに **絶対に** 含めない．
...
利用者の Private リポジトリの存在を本リポジトリの README やコミットメッセージで具体名で明示しない．「個人化版」 「 Private リポジトリ」 のような汎用呼称を使う．
